...
Before reviewing, familiarize yourself with Apollo's Rust best practices. Read chapters ONE AT A TIME as needed — only load a chapter when it is directly relevant to the current task. Do not load multiple chapters in a single turn to avoid exceeding context limits. Reference these files when providing feedback:
